...
import type { EstatusFiltro } from '../ordenes/types';
...
export function FilterModal({ open, startDate, endDate, estatus, onClose, onApply }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-3">
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white p-5 shadow-2xl sm:p-8">
        ...
        <div className="mb-4">
          ...
          <input type="date" value={draftStart} onChange={(e) => setDraftStart(e.target.value)} className="w-full rounded-lg border-2 border-blue-300 px-4 py-2 focus:border-blue-700 focus:outline-none" />
        </div>
        <div className="mb-4">
          <label className="mb-2 block text-sm font-semibold text-blue-900">Hasta:</label>
          <input type="date" value={draftEnd} onChange={(e) => setDraftEnd(e.target.value)} className="w-full rounded-lg border-2 border-blue-300 px-4 py-2 focus:border-blue-700 focus:outline-none" />
        </div>
        <div className="mb-6 rounded-lg bg-blue-50 p-4">
          {OPTIONS.map((opt) => (
            <label key={opt.value} className="mb-2 block last:mb-0">
              <input type="radio" name="filtroHistorial" className="mr-2" checked={draftEstatus === opt.value} onChange={() => setDraftEstatus(opt.value)} />
              {opt.label}
            </label>
          ))}
        </div>
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button type="button" onClick={onClose} className="rounded-lg bg-gray-400 px-6 py-2 text-white hover:bg-gray-500">Cerrar</button>
          <button
            type="button"
