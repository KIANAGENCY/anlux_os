...
export function statusToRadio(estatus: string | null | undefined): 'rojo' | 'naranja' | 'amarillo' | 'verde' {
  const normalized = normalizeStatus(estatus);
...
export function isEntregado(estatus: string | null | undefined): boolean {
  return normalizeStatus(estatus).includes('entregado');
}
