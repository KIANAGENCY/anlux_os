...
## Despliegue en soporte.anlux.mx (cPanel)

Subdominio **`soporte.anlux.mx`** → proyecto **anlux_laravel** (no portal-proyectos).

### 1. Crear subdominio

cPanel → **Subdominios** → `soporte` + `anlux.mx` → carpeta ej. `/home/joses16/soporte.anlux.mx`.
...
```text
/home/joses16/soporte.anlux.mx/public
```

Si la raíz apunta solo a `soporte.anlux.mx/` (sin `public`), el `.htaccess` de la raíz del proyecto redirige a `public/`.

### 3. Subir archivos

Sube **todo** anlux_laravel (app, bootstrap, config, public, vendor, …). En el servidor:
...
DB_CONNECTION=mysql
DB_HOST=localhost
DB_DATABASE=anlux_os
DB_USERNAME=joses16_...
DB_PASSWORD=...

ANLUX_LEGACY_APP_URL=https://anlux.mx
```
...
- `https://soporte.anlux.mx/up` → debe responder OK (health de Laravel)
- `https://soporte.anlux.mx/login` (o ruta de login configurada)

Dominio principal **anlux.mx** puede seguir con otro sitio o legacy; Laravel vive en **soporte**.
...
- revisar `storage/logs/anlux-ops-*.log`
...
- Los errores de correo/PDF quedan registrados en `anlux-ops`.
