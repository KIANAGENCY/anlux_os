...
$outFile = Join-Path $outDir "Informe_Global_Practicas_Exacto.docx"
...
$(Para "")
$(Field "NOMBRE DE LA EMPRESA:" "Expertos en Administración y en Cómputo, S.A. de C.V. (Exacto LP).")
...
$(Para "Se integró ExactoVaultService para cifrar en reposo el teléfono y domicilio del cliente mediante prefijo v1: en base de datos, utilizando secreto configurado en .env (EXACTO_TELEFONO_SECRET). Se implementaron políticas de acceso (OrdenPolicyService, OrderPolicy), middleware de cabeceras de seguridad, registro de actividad en SecurityActivityLogger y módulo de seguridad en /admin/seguridad. Se desarrolló OrdenEditLockService para evitar que dos técnicos editen la misma orden simultáneamente, con heartbeat y liberación de bloqueo vía API.")
