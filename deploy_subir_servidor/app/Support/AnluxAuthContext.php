...
final class AnluxAuthContext
{
    ...
    {
        return (bool) session('anlux_impersonating', false);
    }

    public static function impersonatorId(): ?int
    {
        $id = session('anlux_impersonator_id');
    ...
    public static function nombreTecnicoSesionActual(?User $user = null): string
    {
        ...
        if ($sessionUserId === $userId && $sessionUserId > 0) {
            ...
            if ($desdeSesion !== '') {
                $revealed = app(AnluxVaultService::class)->tecnicoNombreReveal($desdeSesion);
