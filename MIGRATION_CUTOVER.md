...
## Variables `.env` (ver `config/anlux.php`)

- `ANLUX_*` según entorno (firewall, CSP, cookie remember, etc.)
- `ANLUX_LOGIN_ALLOW_PLAINTEXT_PASSWORD` solo migración/debug
- `ANLUX_PEPPER_ROOT` / secretos de vault si aplica

## Cookie «recordar» legacy

- Nombre por defecto: `recuerdame` (`config('anlux.remember_cookie')`)
...
1. **`ANLUX_CUTOVER_BANNER`** — Texto breve (una o varias líneas) mostrado arriba del área de trabajo y en pantallas de invitado (login). HTML no permitido (se escapa); sirve para avisos del tipo «Piloto Laravel — reportar incidencias a …».

2. **`ANLUX_SHOW_LEGACY_ESCAPE_HATCH=true`** — Muestra enlaces a páginas concretas del legacy (`ordenes.php`, `orden_servicio.php`, etc.) que abren en otra pestaña. El técnico o admin debe **iniciar sesión en el legacy** si entra ahí.

3. **`ANLUX_LEGACY_APP_URL`** — URL base del proyecto PHP anterior, **sin barra final** (ej. `http://localhost/anlux` o tu virtual host Laragon).

### Redirección opcional desde el legacy (piloto)

En el proyecto PHP `anlux`, tras cargar variables de entorno, `includes/firewall.php` puede enviar al usuario al front de Laravel si defines en el **`.env` del legacy**:

- `ANLUX_REDIRECT_LEGACY_TO_LARAVEL=1` (u `true`)
- `ANLUX_LARAVEL_PUBLIC_URL` — URL base del `public/` de Laravel, **sin barra final** (ej. `http://127.0.0.1/anlux_laravel/public`)
...
- Mantén el legacy accesible en **otro host o subruta** (ya referenciado en `ANLUX_LEGACY_APP_URL`) para soporte y rollback.

### Checklist previo a go-live

- [ ] `.env` de producción: `APP_URL`, `DB_*`, `ANLUX_LEGACY_APP_URL`, secretos (`ANLUX_TELEFONO_SECRET` si aplica).
...
- [ ] Definir ventana de piloto: activar `ANLUX_CUTOVER_BANNER` y `ANLUX_SHOW_LEGACY_ESCAPE_HATCH` solo si hace falta.

### Rollback rápido

1. Revertir el virtual host o el proxy para que el tráfico principal vuelva al **document root del legacy**.
2. Opcional: desactivar banner y escape hatch en Laravel (`ANLUX_CUTOVER_BANNER` vacío, `ANLUX_SHOW_LEGACY_ESCAPE_HATCH=false`).
3. Comunicar al equipo que la sesión vuelve a ser **solo la del PHP clásico**.

### Tabla de equivalencias Laravel ↔ legacy (referencia)

| Laravel | Legacy (mismo origen que `ANLUX_LEGACY_APP_URL`) |
|--------|-----------------------------------------------------|
