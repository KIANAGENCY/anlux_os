# EXACTO — Copia archivos del plan a deploy_subir_servidor/ (listo para cPanel)
...
if (-not (Test-Path (Join-Path $root 'artisan'))) {
    Write-Error "No se encontró artisan. Ejecuta desde c:\laragon\www\exacto_laravel o ajusta `$root."
}
...
$lines = @(
    'EXACTO — Archivos para subir al servidor',
