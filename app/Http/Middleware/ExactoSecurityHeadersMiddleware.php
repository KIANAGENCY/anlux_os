 ...
 */
final class ExactoSecurityHeadersMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (config('exacto.firewall_disable', false) || config('exacto.firewall_skip_headers', false)) {
            return $response;
        }

        if (! config('exacto.csp_disable', false)) {
            $csp = config('exacto.csp');
        ...
        if ($request->secure()) {
            $maxAge = (int) config('exacto.hsts_max_age', 0);
            if ($maxAge > 0) {
