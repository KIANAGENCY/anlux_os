...
final class OrderEmailService
{
    ...
    public function sendForStatusWithResult(int $idOrdenC, string $estatus, ?array $probeResult = null, ?array $orderPayload = null): array
    {
        ...
        try {
            ...
            [$pdfContent, $pdfFilename] = $this->pdfAttachment($idOrdenC, (string) ($orden['folio'] ?? ''));
     ...
     */
    private function pdfAttachment(int $idOrdenC, string $folio): array
    {
        try {
            ...
            $filename = ($safeFolio !== '' ? $safeFolio : 'orden_'.$idOrdenC).'.pdf';
