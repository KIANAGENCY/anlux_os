 ...
 */
class ExactoSeedTestUsersCommand extends Command
{
    protected $signature = 'exacto:seed-test-users
                            {--database= : Nombre de la base MySQL (por defecto: exacto, o el valor sano si DB_DATABASE apunta a un .sqlite)}';

    protected $description = 'Crea/actualiza admin@exacto.test y tecnico@exacto.test en la tabla login usando la conexión mysql (Laragon), no SQLite.';
    ...
    public function handle(): int
    {
        ...
        if ($mysqlDb === '' || str_contains(strtolower($mysqlDb), '.sqlite')) {
            $mysqlDb = (string) ($this->option('database') ?: 'exacto');
        ...
        try {
            ...
            $GLOBALS['EXACTO_FORCE_TEST_LOGIN_SEED'] = true;
            try {
                ...
                unset($GLOBALS['EXACTO_FORCE_TEST_LOGIN_SEED']);
            }
